            ...
            if (part.type === "text" && part.content) {
              return (
                <div key={index} className="flex items-start gap-2 px-4">
                  {role === "assistant" ? (
                    <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg bg-[var(--lagoon-deep)] font-medium text-white text-xs">
                      AI
                    </div>
                  ) : (
                    <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg bg-[var(--sea-ink-soft)] font-medium text-white text-xs">
                      Y
                    </div>
                  )}
                  <div className="min-w-0 max-w-none flex-1 text-[var(--sea-ink)] text-sm">
    ...
    <div className="relative z-50">
      ...
      {isOpen && (
        <div className="demo-panel fixed inset-x-4 top-20 z-[100] flex h-[calc(100vh-6rem)] max-h-[600px] flex-col overflow-hidden p-0 sm:left-auto sm:w-[min(calc(100vw-2rem),700px)]">
          <div className="flex items-center justify-between border-[var(--line)] border-b p-3">
