...
export function useFormStepper(schemas: ZodObject[]) {
  ...
  const handleCancelOrBack = (opts?: HandleCancelOrBackOpts) => {
    // The last step is also `isCompleted`, so back must key off the step number
    // alone — otherwise the final step's "back" can never return to edit earlier
    // fields. Only the first step has nothing to go back to: that's cancel.
    if (isFirstStep) {
      opts?.onCancel?.();
      return;
    }
